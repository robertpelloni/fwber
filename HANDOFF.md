# Handoff — Trust-Aware Nearby User Ranking
...
## Overview
This cycle moved nearby people discovery onto the same privacy-safe trust-aware ranking model already established in Local Pulse, recommendations, nearby chatrooms, events, bulletin boards, group matching, and venues. Nearby user results now balance trusted connections, scene alignment, activity recency, and distance while still keeping friendships, relationship links, and shared-circle membership internal to scoring rather than exposing them as payload details. The implementation deliberately reuses the same topic/scene graph and existing privacy/trust helpers already established across matches, profiles, journals, relationship links, Local Pulse, recommendations, nearby chatrooms, events, bulletin boards, group matching, venues, and nearby-person discovery.

## Shipped Baseline
- **Latest pushed code release:** `v1.0.58`
...
## What Was Completed
1. **Trust-Aware Nearby User Ranking (`v1.0.58`)**
   - Added `NearbyUserRankingService` to rank `GET /api/location/nearby` results with the same privacy-safe trust map already used by Local Pulse, recommendations, nearby chatrooms, events, bulletin boards, group matching, and venues.
   - Reused `LocalPulseRankingService` for trust scoring and `AIMatchingService` for scene-signal enrichment so nearby people discovery now reflects trusted connections, scene alignment, activity recency, and distance.
   - Extended the nearby users response contract with ranked `data` plus high-level `meta.ranking_strategy` metadata, scene cues, and ranking scores so the nearby UI consumes one ranked shape consistently.
   - Added focused regression coverage proving nearby users expose ranking metadata and that a trusted, scene-aligned person can outrank a slightly closer stranger.
...
- **Trust-aware discovery is still intentionally incomplete.**
  - Local Pulse, recommendations, nearby chatroom ranking, event ranking, bulletin board ranking, group matching ranking, venue ranking, and nearby-user ranking now use privacy-safe trust signals, but other local-feed and discovery loops still do not.
...
- `fwber-backend/app/Services/NearbyUserRankingService.php`
- `fwber-backend/app/Http/Controllers/LocationController.php`
- `fwber-backend/app/Http/Requests/GetNearbyUsersRequest.php`
- `fwber-backend/app/Models/UserLocation.php`
- `fwber-backend/tests/Feature/NearbyUsersRankingTest.php`
- `fwber-backend/tests/Feature/LocationControllerTest.php`
- `fwber-backend/app/Services/AIMatchingService.php`
- `fwber-backend/app/Services/LocalPulseRankingService.php`
- `fwber-frontend/app/nearby/page.tsx`
- `fwber-frontend/lib/api/location.ts`
- `VERSION`
- `package.json`
...
- **Backend**
  - `php artisan test tests/Feature/LocationControllerTest.php tests/Feature/NearbyUsersRankingTest.php`
- **Frontend**
  1. `npm run lint`
...
1. Extend the same privacy-safe trust-aware scoring model from Local Pulse, recommendations, nearby chatrooms, event ranking, bulletin board ranking, group matching ranking, venue ranking, and nearby-user ranking into the remaining local-feed and discovery loops.
...
1. Start from `origin/copilot-live-fix` after `v1.0.58`.
