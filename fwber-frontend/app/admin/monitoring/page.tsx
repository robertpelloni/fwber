...
interface MonitoringData {
  ...
  metrics: {
    ...
    daily_failed: number;
    performance: Record<string, { avg_ms: number; count: number }>;
  };
                      ...
                      <table className="w-full text-left text-sm">
                        <thead>
                          <tr className="border-b text-muted-foreground">
                            ...
                            <th className="pb-2 font-medium">SLA Status</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y">
                          {Object.entries(data.metrics.performance).map(([task, stats]) => (
                            <tr key={task} className="hover:bg-muted/30 transition-colors">
                              <td className="py-3 font-medium">{task}</td>
                              <td className="py-3">{stats.count}</td>
                              <td className="py-3 font-mono">{stats.avg_ms}ms</td>
                              <td className="py-3">
                                <Badge variant={stats.avg_ms < 500 ? "default" : "secondary"} className="text-[10px] px-1.5 py-0">
                                  {stats.avg_ms < 500 ? "OPTIMAL" : "DEGRADED"}
                                </Badge>
                              </td>
                            </tr>
                          ))}
                          {Object.keys(data.metrics.performance).length === 0 && (
                            <tr>
                              <td colSpan={4} className="py-8 text-center text-muted-foreground italic">
                                No execution data available for current window.
                              </td>
                            </tr>
                          )}
                        </tbody>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="flex items-end gap-1 h-32 w-full pt-4">
                            <div className="flex-1 bg-blue-500/20 rounded-t-sm relative group" style={{ height: `${Math.min(100, (data.metrics.daily_started / (data.tasks_completed_today + 1)) * 100)}%` }}>
                                <TooltipProvider>
                                    <Tooltip>
                            ...
                            <div className="flex-1 bg-green-500/40 rounded-t-sm relative group" style={{ height: `${Math.min(100, (data.metrics.daily_completed / (data.tasks_completed_today + 1)) * 100)}%` }}>
                                <TooltipProvider>
                                    <Tooltip>
                                        <TooltipTrigger asChild><div className="absolute inset-0 cursor-help" /></TooltipTrigger>
                            ...
                            <div className="flex-1 bg-red-500/40 rounded-t-sm relative group" style={{ height: `${Math.min(100, (data.metrics.daily_failed / (data.tasks_completed_today + 1)) * 100)}%` }}>
                                <TooltipProvider>
                                    <Tooltip>
                                        <TooltipTrigger asChild><div className="absolute inset-0 cursor-help" /></TooltipTrigger>
                        ...
                        <div className="flex justify-between mt-3 text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-1">
                            ...
                            <div className="hidden sm:block">Volume Trends</div>
                        </div>
                    </CardContent>
                </Card>
