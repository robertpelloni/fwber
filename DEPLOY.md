...
- the signed-in product shell now also includes restored `/events`, `/friends`, `/activity`, `/notifications`, and `/settings/travel` routes, so post-deploy validation should exercise those pages with a real session
