...
- [ ] **Re-run Hetzner Deploy After Repo Ownership Repair**: Confirm the backend deploy workflow is green again after restoring `deploy` ownership of the live checkout.
- [ ] **Deploy Dead-Surface Recovery to Production**: Verify live `/friends`, `/activity`, `/notifications`, and `/settings/travel` with a real signed-in session after the latest green deploys settle.
...
## ✅ Recently Completed
- [x] **Hetzner Repo Ownership Repair**: Restored deploy-user ownership of the live checkout after `.git/objects` permission drift broke the automated Hetzner deploy workflow.
