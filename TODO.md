...
- [ ] **Redeploy After Restoration Phases**: Re-run deployment now that AI, premium/billing, marketplace/merchant, geo-aware merchant discovery, merchant trust scoring, merchant review prioritization, deployment health verification, smoke-check automation, report artifacts, remediation diagnostics, and endpoint fingerprinting are restored on top of the migration-hardening work.
- [ ] **Fix Live Health Route Drift**: The current reachable `api.fwber.me` deployment still returns `404` for `/api/health`, `/api/health/liveness`, and `/api/health/readiness`; current smoke fingerprints show this traffic is landing on **Apache at `75.119.202.57`**.
- [ ] **Fix Live Geo Domain Drift**: The current reachable `geo.fwber.me/nearby` contract returns a Vercel deployment-not-found `404`; current smoke fingerprints show this traffic is landing on **Vercel at `64.29.17.1`** instead of a geo microservice host.
