...
## ✅ Recently Completed
- [x] **Frontend CI Node Runtime Alignment**: Updated the dedicated frontend GitHub workflow to Node.js 24 to match the lockfile generation/runtime family.
- [x] **Discovery Route Recovery**: Hardened `NodeInfoController`, expanded public-route coverage for NodeInfo, and strengthened the Hetzner `api.fwber.me` nginx source config with an explicit `/.well-known/` location.
