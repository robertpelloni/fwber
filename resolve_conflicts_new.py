import sys
import json
import os
import subprocess
import re

def get_conflicted_files():
    result = subprocess.run(['git', 'diff', '--name-only', '--diff-filter=U'], capture_output=True, text=True)
    return [f for f in result.stdout.split('\n') if f.strip()]

def parse_version(v):
    return tuple(map(int, re.findall(r'\d+', v)))

def merge_json(head_json, branch_json):
    if isinstance(head_json, dict) and isinstance(branch_json, dict):
        merged = {}
        for k in set(head_json.keys()).union(branch_json.keys()):
            if k in head_json and k in branch_json:
                if isinstance(head_json[k], dict) and isinstance(branch_json[k], dict):
                    merged[k] = merge_json(head_json[k], branch_json[k])
                elif isinstance(head_json[k], str) and isinstance(branch_json[k], str):
                    if re.search(r'\d+\.\d+', head_json[k]) and re.search(r'\d+\.\d+', branch_json[k]):
                        v1 = parse_version(head_json[k])
                        v2 = parse_version(branch_json[k])
                        merged[k] = head_json[k] if v1 >= v2 else branch_json[k]
                    else:
                        merged[k] = head_json[k] if len(head_json[k]) > len(branch_json[k]) else branch_json[k]
                elif isinstance(head_json[k], list) and isinstance(branch_json[k], list):
                    merged[k] = merge_json(head_json[k], branch_json[k])
                else:
                    merged[k] = head_json[k]
            elif k in head_json:
                merged[k] = head_json[k]
            else:
                merged[k] = branch_json[k]
        return merged
    elif isinstance(head_json, list) and isinstance(branch_json, list):
        res = []
        for item in head_json + branch_json:
            if item not in res:
                res.append(item)
        return res
    else:
        return branch_json

def resolve_file(filepath):
    if not os.path.exists(filepath):
        return
        
    if filepath.endswith('package-lock.json') or filepath.endswith('composer.lock'):
        subprocess.run(['git', 'checkout', '--theirs', filepath])
        return True

    # Get both versions
    head_proc = subprocess.run(['git', 'show', f':2:{filepath}'], capture_output=True, text=True, encoding='utf-8')
    branch_proc = subprocess.run(['git', 'show', f':3:{filepath}'], capture_output=True, text=True, encoding='utf-8')
    
    # If one side is missing, it's an add/delete conflict. Just accept the existing one (ours or theirs)
    if head_proc.returncode != 0:
        subprocess.run(['git', 'checkout', '--theirs', filepath])
        return True
    if branch_proc.returncode != 0:
        subprocess.run(['git', 'checkout', '--ours', filepath])
        return True
        
    head_content = head_proc.stdout
    branch_content = branch_proc.stdout

    # 1. Handle JSON
    if filepath.endswith('.json') or filepath.endswith('.jsonl'):
        try:
            head_json = json.loads(head_content) if head_content.strip() else {}
            branch_json = json.loads(branch_content) if branch_content.strip() else {}
            merged = merge_json(head_json, branch_json)
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(merged, f, indent=2)
            return True
        except Exception as e:
            pass # fallback to text

    # 2. Handle Text (TS, JS, MD, Prisma, etc.)
    out_lines = []
    
    head_lines = head_content.splitlines()
    branch_lines = branch_content.splitlines()
    
    # If it's a VERSION file
    if 'VERSION' in filepath or filepath.endswith('VERSION'):
        v1 = parse_version(head_content)
        v2 = parse_version(branch_content)
        merged_text = head_content if v1 >= v2 else branch_content
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(merged_text)
        return True
        
    # We want to keep both sides but deduplicate lines (e.g. identical imports or logic).
    # Since we can't do full AST merge, we append branch to head, but filter out exact line duplicates.
    res = []
    seen = set()
    
    # For markdown lists or simple files, deduplicating everything works well.
    # For code, deduplicating all exact lines might strip closing braces '}'.
    # We should only deduplicate imports or empty lines or common setup lines.
    for l in head_lines + branch_lines:
        s = l.strip()
        if not s: # keep empty lines if previous wasn't empty
            if res and res[-1].strip() != '':
                res.append(l)
            continue
            
        if l.startswith('import ') or l.startswith('export ') or filepath.endswith('.md'):
            if s not in seen:
                seen.add(s)
                res.append(l)
        else:
            # For logic overlap, try to combine them. We just output both, and if they are exactly identical lines, we might keep them if they are braces.
            # But wait, git already showed this as a conflict, so the lines differ or context differs.
            if s not in seen:
                seen.add(s)
                res.append(l)
            elif s in ['}', ']', ');', '};']:
                res.append(l) # allow duplicate closing braces

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write('\n'.join(res) + '\n')

files = get_conflicted_files()
for f in files:
    print(f"Resolving {f}...")
    resolve_file(f)
