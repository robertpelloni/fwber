...
# Jest resolver doesn't like ../src/app.js. Let's use the actual app.ts path
# or skip it by just making an express app in the test file itself to test the router directly.
...
with open('fwber-backend-ts/tests/QuestVerification.test.ts', 'w') as f:
    f.write(replacement)
